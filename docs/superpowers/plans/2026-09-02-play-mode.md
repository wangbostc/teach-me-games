...
- **No SAN in learner-facing output** — plain square names only (existing `report/render.py` convention; reused verbatim for the report step, and for every Learning Mode candidate's displayed move text via `describe_uci` — not just its fallback).
...
- Produces: `claude_available() -> bool`; `build_options(board: chess.Board, candidates: tuple[Candidate, ...]) -> list[dict]`, each dict `{"uci": str, "move_text": str, "eval_text": str, "explanation": str}`. `move_text` is plain square language via `describe_uci` -- **never SAN**, per this plan's Global Constraints ("No SAN in learner-facing output"). Never raises; a broken/unavailable/invalid explanation degrades to a struct-only fallback string rather than blocking the turn.
